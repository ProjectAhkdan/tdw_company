        ...
        <div className="flex gap-1 rounded-xl border p-1 overflow-x-auto"
          ...
          {["all", ...categories].map(c => (
            <button key={c} onClick={() => setCat(c)}
              ...
              style={cat === c ? { background: GOLD, color: "oklch(0.08 0 0)" } : { color: "oklch(0.55 0.01 60)" }}>
              {c === "all" ? "Semua" : c}
            </button>
          ))}
        </div>
          ...
          {filtered.map(post => (
            <Link key={post.id} href={`/blog/${post.slug}`}
              ...
              <div className="relative h-44 w-full overflow-hidden"
                ...
                {post.thumbnail_url ? (
                  <Image src={post.thumbnail_url} alt={post.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                ) : (
